...
<body data-language="${safeLanguage}">
  <header class="route-header">
    <div id="connectionBadge" class="route-meta" role="button" tabindex="0" aria-label="Mở trung tâm kết nối" aria-live="polite"><span class="connection-emblem" aria-hidden="true"><span id="connectionBrand" class="connection-brand"></span><span id="connectionDot" class="dot"></span></span><span class="connection-copy"><strong id="connectionLabel">Đang kiểm tra</strong></span></div>
    <nav class="header-actions" aria-label="Điều hướng RelayCode">
      <button id="topConnect" class="header-action connect-action" aria-label="Kết nối provider"><span id="topConnectIcon" aria-hidden="true"></span><span id="topConnectLabel" class="header-action-label">Kết nối</span></button>
      <button id="historyToggle" class="header-action" aria-label="Lịch sử chat"><span id="historyToggleIcon" aria-hidden="true"></span><span class="header-action-label">Lịch sử</span></button>
      <button id="metricsToggle" class="header-action" aria-label="Số liệu sử dụng"><span id="metricsToggleIcon" aria-hidden="true"></span><span class="header-action-label">Số liệu</span></button>
      <button id="settings" class="header-action icon-only" aria-label="Cài đặt"><span id="settingsIcon" aria-hidden="true"></span></button>
    </nav>
  </header>

  <section id="historyPanel" class="history-panel hidden" role="dialog" aria-labelledby="historyTitle"><div class="history-heading"><div class="history-heading-copy"><span id="historyHeadingIcon" class="history-heading-icon" aria-hidden="true"></span><div><strong id="historyTitle">Lịch sử chat</strong><span id="historyCount" class="history-count"></span></div></div><div class="history-heading-actions"><button id="clearAllHistory" class="history-clear-all hidden" type="button">Xóa tất cả</button><button id="closeHistory" type="button" aria-label="Đóng"></button></div></div><div id="historyList" class="history-list"></div><button id="viewAllHistory" class="history-view-all hidden" type="button">View all</button></section>
  ...
  <section id="configPanel" class="config-panel hidden">
    <div class="config-heading"><div><strong id="configHeadingTitle">Cấu hình provider</strong><span id="configHeadingCopy">Chọn nguồn model cho mọi yêu cầu</span></div><button id="closeConfig" class="header-action">Đóng</button></div>
    <div id="configScroll" class="config-scroll">
      <section class="config-section config-interface-section"><div class="config-section-heading"><strong id="configInterfaceTitle">Giao diện</strong><span id="configInterfaceCopy">Ngôn ngữ dùng trong RelayCode</span></div><label class="language-setting"><span id="interfaceLanguageLabel">Ngôn ngữ giao diện</span><div id="languagePicker" class="language-picker"><button id="languageTrigger" class="language-trigger" type="button" aria-haspopup="listbox" aria-expanded="false"><span id="uiLanguageLabel">Tiếng Việt</span><i aria-hidden="true"></i></button><div id="languageMenu" class="language-menu settings-menu-floating hidden" role="listbox" popover="manual"><button type="button" data-language="vi"><strong>Tiếng Việt</strong><small>Giao diện tiếng Việt</small></button><button type="button" data-language="en"><strong>English</strong><small>English interface</small></button></div><select id="uiLanguage" class="hidden" aria-hidden="true" tabindex="-1"><option value="vi">Tiếng Việt</option><option value="en">English</option></select></div></label></section>
      <section class="config-section"><div class="config-section-heading"><strong id="configProfilesTitle">Hồ sơ</strong><span id="configProfilesCopy">Lưu nhiều cấu hình provider riêng biệt</span></div><div class="profile-bar"><div class="profile-picker" id="profilePicker"><span id="activeProfileText">Hồ sơ đang dùng</span><button id="profileTrigger" class="profile-trigger" type="button" aria-haspopup="listbox" aria-expanded="false"><span id="activeProfileLabel">Chưa chọn hồ sơ</span><i aria-hidden="true"></i></button><div id="profileMenu" class="profile-menu settings-menu-floating hidden" role="listbox" popover="manual"></div></div><div class="profile-actions"><button id="deleteProfile" class="profile-delete" disabled>Xóa hồ sơ</button><button id="newProfile" class="secondary profile-new">+ Hồ sơ mới</button></div></div><label><span id="profileNameText">Tên hồ sơ</span><input id="profileName" spellcheck="false" placeholder="Ví dụ: OpenAI cá nhân"></label></section>
      <section class="config-section"><div class="config-section-heading"><strong id="configConnectionTitle">Kết nối</strong><span id="configConnectionCopy">Provider và địa chỉ API đang dùng</span></div><div class="provider-field"><span id="providerFieldText">Provider</span><div class="provider-picker" id="providerPicker"><button id="providerTrigger" class="provider-trigger" type="button" aria-haspopup="listbox" aria-expanded="false"><span id="providerBrand" class="provider-brand-slot"></span><span><strong id="providerLabel">9Router</strong><small id="providerHint">Gateway local, nhiều model</small></span></button><div id="providerMenu" class="provider-menu settings-menu-floating hidden" role="listbox" popover="manual"><button type="button" class="provider-option" data-provider="9router"><span><strong>9Router</strong><small>Gateway local, nhiều model</small></span></button><button type="button" class="provider-option" data-provider="cockpit"><span><strong>Cockpit Tools</strong><small>Gateway local · nhiều tài khoản</small></span></button><button type="button" class="provider-option" data-provider="opencode"><span><strong>OpenCode</strong><small>OpenCode Zen · OpenAI-compatible</small></span></button><button type="button" class="provider-option" data-provider="openai"><span><strong>OpenAI</strong><small>API chính thức · cần API key</small></span></button><button type="button" class="provider-option" data-provider="anthropic"><span><strong>Anthropic Claude</strong><small>Messages API · cần API key</small></span></button><button type="button" class="provider-option" data-provider="openai-compatible"><span><strong>OpenAI-compatible</strong><small>Endpoint tùy chỉnh</small></span></button><button type="button" class="provider-option" data-provider="ollama"><span><strong>Ollama</strong><small>Local · không cần API key</small></span></button><button type="button" class="provider-option" data-provider="lm-studio"><span><strong>LM Studio</strong><small>Local · không cần API key</small></span></button></div></div><select id="configProvider" class="hidden" aria-hidden="true" tabindex="-1"><option value="9router">9Router</option><option value="cockpit">Cockpit Tools</option><option value="opencode">OpenCode</option><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="openai-compatible">OpenAI-compatible</option><option value="ollama">Ollama local</option><option value="lm-studio">LM Studio local</option></select></div><label><span id="configEndpointLabel">Endpoint</span><input id="configEndpoint" spellcheck="false" placeholder="http://127.0.0.1:20128/v1"></label><label id="apiKeyField"><span id="apiKeyLabel">API key</span><input id="configApiKey" type="password" autocomplete="off" placeholder="Nhập API key của provider"></label><p id="keyState" class="key-state">Chưa lưu API key</p></section>
      <section class="config-section config-price-section"><div class="config-section-heading"><strong id="configCostTitle">Chi phí ước tính</strong><span id="configCostCopy">Không bắt buộc, tính theo một triệu token</span></div><div class="price-row"><label>Input $ / 1M<input id="inputPrice" type="number" min="0" step="0.01" placeholder="Tùy chọn"></label><label>Output $ / 1M<input id="outputPrice" type="number" min="0" step="0.01" placeholder="Tùy chọn"></label></div></section>
    </div>
    <div class="config-action-dock"><div class="config-actions"><button id="saveConfig" class="primary">Lưu và kết nối lại</button><button id="runDiagnostics" class="secondary">Chẩn đoán</button></div><div class="config-subactions"><button id="openMcp" class="secondary">MCP tools</button><button id="exportDiagnostics" class="secondary">Xuất chẩn đoán</button><button id="openOmniRoute" class="secondary hidden">Mở OmniRoute</button><button id="openCockpit" class="secondary hidden">Mở Cockpit</button><button id="localSetup" class="secondary hidden">Thiết lập local</button></div><p id="diagnosticsResult" class="diagnostics-result hidden" role="status" aria-live="polite"></p></div>
  </section>
  ...
  <section id="setup" class="setup hidden">
    ...
    <div class="launch-panel">
      ...
      <div class="launch-state">
        <span class="state-slot" aria-hidden="true"><span class="state-mark"></span></span>
      ...
      <p id="setupCheckResult" class="setup-check-result">Chưa kiểm tra sức khỏe API trong phiên này.</p>
    </div>
  ...
  <main id="console" class="console hidden">
    ...
    <footer class="composer-shell">
      ...
      <div id="codexTuning" class="codex-tuning hidden">
        ...
        <button id="quotaReset" class="quota-reset hidden" type="button"><span>Reset</span><strong id="quotaResetLabel"></strong></button>
      </div>
      <div id="contextMeter" class="context-meter"><i></i></div>
      <div class="composer-actions"><button id="attach" class="tool-button plus-button" type="button" aria-label="Mở menu thêm" aria-haspopup="menu" aria-expanded="false"><span id="attachIcon" aria-hidden="true"></span></button><span id="attachmentProgress" class="attachment-progress hidden"><i></i>Đang tải</span><div id="goalDock" class="goal-dock hidden"><button id="goalDockTrigger" class="goal-dock-trigger" type="button" aria-haspopup="dialog" aria-expanded="false"><span id="goalDockIcon" aria-hidden="true"></span><strong>Goal</strong></button><section id="goalRail" class="goal-rail hidden" aria-live="polite"><span id="goalState" class="goal-state" aria-hidden="true"></span><div class="goal-copy"><strong id="goalTitle">Goal đang bật</strong><span id="goalStatus">Yêu cầu tiếp theo sẽ trở thành mục tiêu</span></div><div class="goal-actions"><button id="goalPause" class="hidden" type="button">Tạm dừng</button><button id="goalResume" class="hidden" type="button">Tiếp tục</button><button id="goalClear" type="button" aria-label="Tắt Goal">×</button></div></section></div><div class="mode-picker" id="modePicker"><button id="modeTrigger" class="mode-trigger" type="button" aria-haspopup="listbox" aria-expanded="false"><span id="modeLabel">Agent</span></button><div id="modeMenu" class="mode-menu hidden" role="listbox"><button data-mode="agent" class="active"><strong>Agent</strong><small>Đọc, sửa file và chạy lệnh</small></button><button data-mode="chat"><strong>Chat</strong><small>Trò chuyện trực tiếp với model</small></button><button data-mode="plan"><strong>Plan</strong><small>Lập kế hoạch trước khi hành động</small></button></div></div><div class="perm-wrap" id="permDropdown"><button class="perm-trigger" id="permissionMode" type="button" data-mode="ask" aria-haspopup="listbox" aria-expanded="false"><span class="perm-trigger-icon" aria-hidden="true"><svg class="perm-icon-ask" viewBox="0 0 24 24"><path d="M8 11V7a2 2 0 1 1 4 0v3m0-2V5a2 2 0 1 1 4 0v5m0-2V6a2 2 0 1 1 4 0v8a6 6 0 0 1-6 6h-2a6 6 0 0 1-4.24-1.76L4 14.49a2 2 0 0 1 2.83-2.83L8 13V11Z"/></svg><svg class="perm-icon-edit" viewBox="0 0 24 24"><path d="m4 16.5-.7 4.2 4.2-.7L19.2 8.3a2.1 2.1 0 0 0-3-3L4.5 16.5Z"/><path d="m14.9 6.6 3 3"/></svg><svg class="perm-icon-full" viewBox="0 0 24 24"><path d="M12 3 19 6v5c0 4.6-2.9 8.2-7 10-4.1-1.8-7-5.4-7-10V6l7-3Z"/><path d="m9 12 2 2 4-4"/></svg></span><span id="permLabel">Hỏi</span><span class="perm-arrow" aria-hidden="true"></span></button><div class="perm-menu hidden" id="permMenu" role="listbox" aria-label="Quyền thao tác"><button class="perm-opt active" data-perm="ask" role="option"><span class="perm-choice-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M8 11V7a2 2 0 1 1 4 0v3m0-2V5a2 2 0 1 1 4 0v5m0-2V6a2 2 0 1 1 4 0v8a6 6 0 0 1-6 6h-2a6 6 0 0 1-4.24-1.76L4 14.49a2 2 0 0 1 2.83-2.83L8 13V11Z"/></svg></span><span class="perm-choice-copy"><strong>Hỏi mọi thao tác</strong><small>Luôn hỏi trước khi thực hiện</small></span><span class="perm-check" aria-hidden="true">✓</span></button><button class="perm-opt" data-perm="edit" role="option"><span class="perm-choice-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m4 16.5-.7 4.2 4.2-.7L19.2 8.3a2.1 2.1 0 0 0-3-3L4.5 16.5Z"/><path d="m14.9 6.6 3 3"/></svg></span><span class="perm-choice-copy"><strong>Cho phép sửa file</strong><small>Chỉ hỏi khi chạy lệnh</small></span><span class="perm-check" aria-hidden="true">✓</span></button><button class="perm-opt perm-opt-full" data-perm="full" role="option"><span class="perm-choice-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 3 19 6v5c0 4.6-2.9 8.2-7 10-4.1-1.8-7-5.4-7-10V6l7-3Z"/><path d="m9 12 2 2 4-4"/></svg></span><span class="perm-choice-copy"><strong>Full access</strong><small>Không hỏi lại khi Agent hoạt động</small></span><span class="perm-check" aria-hidden="true">✓</span></button></div></div><div class="model-picker" id="modelPicker"><button id="modelTrigger" class="model-trigger" type="button"><span id="modelBrand" class="model-trigger-brand"></span><span id="modelLabel">Chọn model</span></button><div id="modelMenu" class="model-menu hidden"><div class="model-menu-heading"><strong id="modelMenuTitle">Models</strong><span id="modelMenuCopy">Chọn model cho yêu cầu tiếp theo</span></div><input id="modelSearch" placeholder="Tìm model…"><button id="checkModels" class="model-check-all" type="button">Kiểm tra model</button><div id="modelHealthFilters" class="model-health-filters hidden" role="group" aria-label="Lọc kết quả kiểm tra model"><button type="button" data-model-health-filter="all" class="active" aria-pressed="true"><span id="modelFilterAllLabel">Tất cả</span><b id="modelFilterAllCount">0</b></button><button type="button" data-model-health-filter="ok" aria-pressed="false"><span id="modelFilterOkLabel">Chạy được</span><b id="modelFilterOkCount">0</b></button><button type="button" data-model-health-filter="error" aria-pressed="false"><span id="modelFilterErrorLabel">Có lỗi</span><b id="modelFilterErrorCount">0</b></button></div><div id="modelOptions" class="model-options"></div></div></div><select id="model" class="hidden" aria-label="Chọn model"><option value="">Chọn model</option></select><button id="send" class="send" aria-label="Gửi" disabled><span id="sendIcon" class="send-icon" aria-hidden="true"></span></button></div>
    </footer>
  </main>
  <div id="connectionDiagnostics" class="modal-backdrop connection-dialog-backdrop hidden"><section class="connection-dialog" role="dialog" aria-modal="true" aria-labelledby="connectionDialogTitle" aria-describedby="connectionDialogSubtitle connectionMessage" aria-busy="false"><div class="connection-dialog-head"><span id="connectionDialogIcon" class="connection-dialog-icon" aria-hidden="true"></span><div><strong id="connectionDialogTitle">Kiểm tra kết nối</strong><span id="connectionDialogSubtitle">Đang kiểm tra provider hiện tại</span></div><button id="closeConnectionDiagnostics" type="button" aria-label="Đóng"></button></div><div class="connection-dialog-body"><div class="connection-provider"><span id="connectionProviderMark">AI</span><div><strong id="connectionProviderName">Provider</strong><small id="connectionEndpoint">Chưa có endpoint</small></div><b id="connectionHealthBadge" class="checking" role="status" aria-live="polite">Đang kiểm tra</b></div><div class="connection-stats"><div><span>Độ trễ</span><strong id="connectionLatency">-</strong></div><div><span>Models</span><strong id="connectionModels">-</strong></div></div><p id="connectionMessage" class="connection-message" role="status" aria-live="polite">Đang gửi yêu cầu kiểm tra…</p></div><div class="connection-dialog-actions"><button id="openConnectionSettings" class="secondary" type="button">Cấu hình</button><button id="retryDiagnostics" class="primary" type="button">Kiểm tra lại</button></div></section></div>
  <div id="uiDialog" class="modal-backdrop ui-dialog-backdrop hidden">
    <section class="ui-dialog" role="dialog" aria-modal="true" aria-labelledby="uiDialogTitle" aria-describedby="uiDialogMessage uiDialogDetail" tabindex="-1">
      <header class="ui-dialog-head"><span id="uiDialogIcon" class="ui-dialog-icon" aria-hidden="true"></span><div class="ui-dialog-heading"><strong id="uiDialogTitle"></strong><p id="uiDialogMessage"></p></div><button id="uiDialogClose" type="button" aria-label="Đóng"></button></header>
      <div class="ui-dialog-body"><div id="uiDialogDetailWrap" class="ui-dialog-detail hidden"><span id="uiDialogDetailIcon" aria-hidden="true"></span><small id="uiDialogDetail"></small></div><label id="uiDialogField" class="ui-dialog-field hidden"><span id="uiDialogFieldLabel"></span><input id="uiDialogInput" autocomplete="off" aria-describedby="uiDialogError"><small id="uiDialogError" class="hidden"></small></label></div>
