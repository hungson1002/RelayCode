...
.filters{grid-template-columns:repeat(4,minmax(0,1fr));gap:7px;margin:0}.dashboard-select{position:relative;min-width:0}.dashboard-select>select{display:none}.select-trigger{display:flex;align-items:center;justify-content:space-between;gap:9px;width:100%;height:35px;padding:0 11px;border:1px solid #393e43;border-radius:9px;background:#202326;color:#d9dde0;font-size:9.5px;text-align:left;cursor:pointer}.select-trigger:hover,.dashboard-select.open .select-trigger{border-color:#596068;background:#25282b}.select-trigger>span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.select-trigger:after{content:"";width:7px;height:7px;flex:none;border-right:1.5px solid #a7adb2;border-bottom:1.5px solid #a7adb2;transform:rotate(45deg) translateY(-2px)}.dashboard-select.open .select-trigger:after{transform:rotate(225deg) translate(-2px,-2px)}.select-menu{position:absolute;z-index:20;top:calc(100% + 6px);left:0;right:0;display:grid;gap:2px;max-height:min(320px,45vh);overflow:auto;padding:6px;border:1px solid #454b51;border-radius:11px;background:#202326;box-shadow:0 18px 46px rgba(0,0,0,.58);scrollbar-width:none}.select-menu::-webkit-scrollbar{display:none;width:0}.select-menu button{width:100%;min-height:32px;padding:7px 9px;border:0;border-radius:7px;background:transparent;color:#cfd3d6;font-size:9.5px;text-align:left;white-space:normal;cursor:pointer}.select-menu button:hover,.select-menu button.active{background:#303438;color:#fff}.select-menu button.active:after{content:"✓";float:right;color:var(--accent);font-weight:700}
.section-head{align-items:center;margin:0 2px 9px}.section-head h2{font-size:11px}.section-head span{font-size:8.5px}
.accounts{gap:14px}.account{border-color:#34383d;border-radius:16px;background:#181a1c;overflow:hidden}.account-head{grid-template-columns:38px minmax(0,1fr) auto auto;gap:12px;padding:13px 16px;border-bottom-color:#303438;background:#1b1d1f}.account-brand{display:grid;place-items:center;width:38px;height:38px;padding:7px;border-color:#3b4146;border-radius:10px;background:#111314}.account-brand svg,.account-brand img{display:block;width:22px!important;height:22px!important;max-width:22px!important;max-height:22px!important;object-fit:contain}.account-name strong{font-size:11px}.account-name span{margin-top:1px;font-size:8.5px}.plan{padding:4px 7px;border:1px solid #34393e;border-radius:6px;background:#202326}.state{font-size:8.5px}
...
<div class="section-head"><h2>Quota by account</h2><span>${displayedQuota.fetchedAt ? `Cập nhật ${safe(relativeTime(displayedQuota.fetchedAt))}` : 'Chưa có rate limit mới'}</span></div>
<section id="accounts" class="accounts">${displayedQuota.accounts.map(accountCard).join('') || emptyState(displayedQuota)}</section>
<section class="history"><div class="section-head"><h2>Recent model activity</h2><span>${visibleRecords.length} request được lưu cục bộ</span></div>
<div class="request-table"><div class="request-head"><span>Model</span><span>Hồ sơ</span><span>Token</span><span>Latency</span></div>${requestRows || '<div class="empty"><div><strong>Chưa có request</strong><span>Số liệu xuất hiện sau lần dùng Chat hoặc Agent đầu tiên.</span></div></div>'}</div></section>
